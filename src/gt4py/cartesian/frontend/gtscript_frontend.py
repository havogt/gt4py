...
class CallInliner(ast.NodeTransformer):
    ...
    def visit_Assign(self, node: ast.Assign):
        if isinstance(node.value, ast.Call):
            ...
            if gt_meta.get_qualified_name_from_node(node.value.func) not in (
                gtscript.MATH_BUILTINS
                | gtscript.TYPE_HINT_AND_CAST_BUILTINS
    ...
    def visit_Expr(self, node: ast.Expr):
        ...
        if (
            ...
            and gt_meta.get_qualified_name_from_node(node.value.func) not in builtin_names
        ):
