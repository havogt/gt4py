...
def _flatten_tuple_fields(args: Sequence[Any]) -> Sequence[Any]:
    flat_args = tuple(
        [arg]
        if isinstance(arg, gtx_common.Field) or np.isscalar(arg)
        else _flatten_tuple_fields(arg)
        for arg in args
    )
    return list(itertools.chain(*flat_args))


def _get_args(sdfg: dace.SDFG, args: Sequence[Any]) -> dict[str, Any]:
    sdfg_params: Sequence[str] = sdfg.arg_names
    flat_args = _flatten_tuple_fields(args)
    return {
