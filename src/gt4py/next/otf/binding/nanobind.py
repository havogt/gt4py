...
class BindingCodeGenerator(TemplatedGenerator):
    ...
    def visit_BufferSID(self, sid: BufferSID, **kwargs: Any) -> str:
        ...
        stride_spec = [dim.static_stride if dim.static_stride is not None else -1 for dim in sid.dimensions]
        stride_spec_string = f"gridtools::nanobind::stride_spec<{', '.join(str(s) for s in stride_spec)}>{{}}"
...
def make_argument(name: str, type_: ts.TypeSpec) -> str | BufferSID | Tuple:
    if isinstance(type_, ts.FieldType):
        return BufferSID(
            source_buffer=name,
            dimensions=[DimensionType(name=dim.value, static_stride=1 if dim.kind == common.DimensionKind.HORIZONTAL else None) for dim in type_.dims],
