...
def concat_where(
    ...
    The condition must be one-dimensional:
    - A 1D ``Domain`` (e.g. ``I < 5``): selects a contiguous region along one dimension.
    - A ``tuple[Domain, ...]`` of 1D same-dimension Domains (e.g. from ``I != 3``):
      selects multiple disjoint regions along one dimension.

    Args:
        cond: 1D Domain or tuple of 1D same-dimension Domains specifying the "true" region.
