...
class NamespaceProxy(ts.TypeSpec):
    ...
    def __getattr__(self, key: str) -> ts.TypeSpec:
        ...
        #      `__setstate__`, `__reduce_ex__`, `__deepcopy__` on freshly-constructed
        #      instances *before* `_object` has been restored. Forwarding them to
        #      `self._object` is both wrong and unbounded — `self._object` itself
        #      would re-enter this method since `_object` has not been assigned yet.
        #   2. Any other attribute that arrives here before `_object` is bound:
        #      use `object.__getattribute__` to consult the actual instance dict
        #      so we reach base-class `AttributeError` rather than recursing.
        if key.startswith("_"):
