    ...
    for key, domain in additional_domains.items():
        ...
        if original_domain is None:
            ...
            new_domains[key] = tuple(
                ...
                for d1, d2 in itertools.zip_longest(original_domain, domain)
            )
...
def infer_program(
    ...
    if len(program.body) > 1:
        raise NotImplementedError("Only single statement programs are supported.")
    ...
    for set_at in reversed(program.body):
        ...
        the_domain = SymbolicDomain.from_expr(set_at.domain)
        transformed_call, current_accessed_domains = infer_expr(
            set_at.expr, the_domain, offset_provider
        )
        transformed_set_ats.insert(
            0,
            itir.SetAt(
                expr=transformed_call,
                domain=SymbolicDomain.as_expr(the_domain),  # type: ignore[arg-type]  # ensured by if condition
