...
<div class="stats">
<div class="stat"><div class="n">{ref16/med('fwd','padded','multi:8x4',16384):.0f}×</div><div class="l">forward step on 32 GPUs at 16384² against the plain single-GPU model (best case: padded on the 8×4 block, {fmt(med('fwd','padded','multi:8x4',16384))} ms vs {fmt(ref16)} ms). Against the same sharded model on one GPU the best transport reaches {N['sp_c32']:.0f}× (coloured 2-phase, 91% efficiency)</div></div>
...
<li><b>Forward strong scaling is near-ideal to 8 GPUs and holds to 32 for the right transport.</b> Speed-ups are quoted against the same sharded model on one GPU, which isolates how the exchange scales; against the plain single-GPU model, which is what a user gains, subtract the sharded formulation's own P=1 overhead of {N['overhead16']:.2f}× (padded) to 1.71× (coloured 2-phase). At 16384² the coloured 2-phase transport reaches {N['sp_c32']:.1f}× on 32 GPUs ({ref16/c32:.1f}× against the reference); padded reaches {N['sp_p32']:.1f}× on the 32×1 strip and {base16/med('fwd','padded','multi:8x4',16384):.1f}× on the 8×4 block ({ref16/p32:.1f}× and {ref16/med('fwd','padded','multi:8x4',16384):.1f}× against the reference), because its dense all-to-all sends a full-size slot to every peer: its wire volume grows with the GPU count and with the row length, while the coloured transports move exactly the halo.</li>
...
<p>Two baselines are in play throughout this report. The speed-ups and efficiencies in the tables are relative to the <em>same sharded model with the same transport on one GPU</em>; they measure how the exchange and the decomposition scale. A user gains less than that, because the sharded formulation is slower than the plain model already on one GPU: at 16384² the plain reference step takes {fmt(ref16)} ms, the padded sharded step {fmt(base16)} ms ({N['overhead16']:.2f}×) and the coloured 2-phase step {fmt(med('fwd','coloured2ph','single:1x1',16384))} ms ({med('fwd','coloured2ph','single:1x1',16384)/ref16:.2f}×), the difference being the pad, select and ppermute bookkeeping on the halo-extended array with nothing to exchange. The last column of each table therefore also gives the speed-up against the plain single-GPU model.</p>
<p>Forward step at 16384² (speed-up over the same transport on one GPU, parallel efficiency; last column against the plain single-GPU reference):</p>
{node_table('fwd', 16384)}
<p>Rematerialised gradient at 8192² (the largest size that runs on one GPU; last column against the plain single-GPU reference gradient, which is {fmt(ref['ref_grad_remat']['4096']['med'])} ms at 4096² and did not fit at 8192², so the reference is scaled by four):</p>
