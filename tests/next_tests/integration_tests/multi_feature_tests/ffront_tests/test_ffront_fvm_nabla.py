...
def test_ffront_compute_zavgS(fieldview_backend, jit):
    allocator = fieldview_backend
    if jit and not isinstance(fieldview_backend, allocators.StandardJAXCPUFieldBufferAllocator):
        pytest.skip("JIT only supported for JAX backend")
    fieldview_backend = (
    ...
    compute_zavgS.with_backend(fieldview_backend).with_jit(jit)(
