...
try:
    from gt4py.next.program_processors.runners.jax_jit import run_jax_jit
...
pytestmark = pytest.mark.skipif(run_jax_jit is None, reason="JAX is not installed")
...
def test_arithmetic(jax_cartesian_case):
    ...
    cases.verify_with_default_data(
        jax_cartesian_case, testee, ref=lambda a, b: a * b - b / a
    )
