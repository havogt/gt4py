...
def test_gather_premap_introduced_vertical_dim_canonicalized():
    # Introducing a vertical dim (K) where a local dim (Band) survives (the RRTMGP Planck pattern):
    # in-place replacement yields the non-canonical (Cell, K, Band); the output must be canonical.
    PT = Dimension("PT")
    ...
    # canonical order: Band (local) before K (vertical), not the in-place (Cell, K, Band)
    assert result.domain == common.Domain(
